...
- Google Search Console HTML verification is complete. Sitemap submission was accepted; Search Console still reports “could not fetch” for the submitted sitemap; Google live URL inspection fetches the sitemap successfully. The home index request is accepted. Sitemap processing is not verified complete.
- Naver ownership verification has the HTML meta tag deployed, but the final ownership CAPTCHA remains user-pending.
