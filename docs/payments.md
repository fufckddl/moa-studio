...
시크릿 키는 Node 서버에서만 사용합니다. `VITE_` 접두어를 붙이거나 브라우저
...
않습니다. 주문과 승인 결과는 기존 SQLite DB에 저장하며 같은 주문은 중복 반영하지
않습니다. 승인 응답이 불확실하면 토스 주문 조회 API로 확인합니다. 결제 내역
...
계약이 완료된 가맹점의 라이브 키, HTTPS 운영 주소 `PUBLIC_APP_URL`,
`MOA_SECURE_COOKIES=1`, `TOSS_LIVE_ENABLED=1`, `PAID_FEATURES_READY=1`,
`MOA_AI_READY=1`이 모두 필요합니다. `PAID_FEATURES_READY`는 유료 권한·기간·운영
정책·환불/지원 절차가 준비된 뒤 켜고, `MOA_AI_READY`는 유료 플랜의 AI 제공이
실제로 검증된 뒤 켭니다. 운영 웹 서버는 `/api`를 Node 서버로 전달하고 나머지는
SPA로 제공해야 합니다. DB를 영구 저장하고 백업하세요. 운영 배포 파일은
준비했으며 [배포 안내](deployment.md)를 따릅니다. 운영 서버·도메인이 아직 없어
실제 배포와 라이브 결제는 비활성화 상태입니다.
