...
function PhotoChatPanel({ photo, cardTitle, cardPreview, onReplace, disabled, open, onClose, cardId, userId, onBusyChange, width, viewportWidth, onWidthChange }: Props & { open: boolean; onClose: () => void; width: number; viewportWidth: number; onWidthChange: (width: number) => void }) {
  ...
  const [personPickerError, setPersonPickerError] = useState('');
  ...
  useEffect(() => {
    if (!open) setPersonPickerOpen(false);
  }, [open]);
  ...
  function updateScrollPosition() {
    const area = scrollArea.current;
    if (!area) return;
    scrollWasNearBottom.current = area.scrollHeight - area.scrollTop - area.clientHeight < 32;
  }
  ...
  useEffect(() => {
    ...
    const observer = new ResizeObserver(() => {
      if (scrollWasNearBottom.current && !candidateRef.current) scrollToLatest();
    });
  ...
  }, [onClose, open, personPickerOpen]);

  function closePersonPicker() {
    setPersonPickerOpen(false);
    setPersonPickerError('');
    requestAnimationFrame(() => pickerToggle.current?.focus());
  }
  ...
  function usePersonReference(person: SavedPerson, focusOnFailure = true) {
    ...
    if (references.some(reference => reference.id === id)) {
      ...
      if (focusOnFailure) requestAnimationFrame(() => input.current?.focus());
      return false;
    }
  ...
  function usePersonFromPicker(person: SavedPerson) {
    setPersonPickerError('');
    if (usePersonReference(person, false)) {
      setPersonPickerOpen(false);
      return;
    }
    setPersonPickerError(references.some(reference => reference.id === personReferenceId(person.id))
      ? `${person.name}은 이미 인물 참고로 추가되어 있어요.`
      : '참고 이미지는 최대 3장까지 추가할 수 있어요. 기존 참고 이미지를 지운 뒤 다시 선택해 주세요.');
  }
  ...
  return <aside hidden={!open} id="photo-chat-panel" className={`photo-chat-panel${resizing ? ' is-resizing' : ''}${personPickerOpen ? ' is-picker-open' : ''}`} style={{ width: viewportWidth <= 800 ? '100%' : width }} aria-labelledby="photo-chat-title">
    ...
    <div className="photo-chat-stage">
      <div className="photo-chat-scroll" ref={scrollArea} onScroll={updateScrollPosition}>
        ...
        {visibleCandidate && previewPhoto ? <figure ref={reviewStart} className="photo-chat-image is-review"><img src={previewPhoto.dataUrl} alt={imageAlt} /><figcaption>{!showOriginal ? visibleCandidate.kind === 'generated' ? '생성한 인물 · 카드에는 아직 반영되지 않았어요' : '변경안 · 아직 적용되지 않았어요' : previewPhoto.name}</figcaption></figure> : !photo && <p className="photo-chat-empty">{mode === 'person' ? '인물 생성은 원본 사진 없이 만들 수 있어요. 만든 인물은 AI 사진 수정의 참고로 사용됩니다.' : '먼저 카드에 사용할 사진을 추가해 주세요.'}</p>}
      ...
      {personPickerOpen && <PersonLibrary id="photo-chat-person-picker" people={people} draft={generatedDraft} state={peopleState} error={peopleError} actionError={personPickerError} disabled={disabled || busy} onClose={closePersonPicker} onRetry={() => void loadPersonLibrary()} onRetryDraft={(draft) => void saveGeneratedPerson(draft)} onUse={usePersonFromPicker} onRename={(person) => void renameLibraryPerson(person)} onDelete={(person) => void deleteLibraryPerson(person)} />}
    </div>
...
function PersonLibrary({ id, people, draft, state, error, actionError, disabled, onClose, onRetry, onRetryDraft, onUse, onRename, onDelete }: {
  id: string;
  people: SavedPerson[];
  ...
  const draftRow = draft && draftPerson ? <article className="photo-chat-person is-draft">
    <button type="button" className="photo-chat-person-select" disabled={disabled} onClick={() => onUse(draftPerson)}>
      <img src={draftPerson.dataUrl} alt={`${draftPerson.name} 인물`} />
      <span>
        <strong>{draftPerson.name}</strong>
        <small>{draft.save.state === 'saving' ? '저장 중' : draft.save.state === 'error' ? '저장 실패' : '저장됨'}</small>
      </span>
    </button>
  ...
  return <section id={id} className="photo-chat-library" aria-label="인물 보관함">
    <div className="photo-chat-library-top">
      <h3>인물 보관함</h3>
      <button className="photo-chat-library-collapse" type="button" onClick={onClose}>접기</button>
    ...
    {actionError && <p className="photo-chat-library-error" role="alert">{actionError}</p>}
    ...
    {(draftPerson || peopleWithoutDraft.length > 0) && <div className="photo-chat-library-list">
      ...
      {peopleWithoutDraft.map(person => <article key={person.id} className="photo-chat-person">
        <button type="button" className="photo-chat-person-select" disabled={disabled} onClick={() => onUse(person)}>
          <img src={person.dataUrl} alt={`${person.name} 인물`} />
          <span>
            <strong>{person.name}</strong>
            <small>{person.createdAt ? new Date(person.createdAt).toLocaleDateString('ko-KR', { timeZone: 'Asia/Seoul' }) : '저장됨'}</small>
          </span>
        </button>
        <div className="photo-chat-person-actions">
